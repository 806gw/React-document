...
```js const element = <Welcome name="Sara" />;```
...
```
[CodePen에서 시험해보기](https://codepen.io/gaearon/pen/KgQKPM?editors=1010)
...
prop의 이름은 사용될 context가 아닌 컴포넌트 자체의 관점에서 짓는 것을 권장합니다.
...
```
[CodePen에서 시험해보기](https://codepen.io/gaearon/pen/MjQWQO?editors=1010)
...
```
이런 함수들은 순수 함수라고 호칭합니다. 입력값을 바꾸려하지 않고 항상 동일한 입력값에 대해 동일한 결과를 반환하기 때문입니다.
